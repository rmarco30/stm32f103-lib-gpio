 ...
 * @param    gpio_params: pointer to GPIO_Params_t structure where desired GPIO
 *                        function was initialized.
 * @retval   none
 */
void gpio_init(GPIO_TypeDef* GPIOx, uint8_t GPIO_PINx, GPIO_Params_t* gpio_params)
{
    /* check if alternate function is required */
    uint32_t alt_func_en = 0;

    if( (gpio_params->MODE != 0) && (gpio_params->CONFIG > 0x01) )
    {
        alt_func_en = 0x01;
    }
    ...
    if( (gpio_params->MODE == 0)  && (gpio_params->CONFIG > 0x01) )
    {
        ...
        else
        {
            ...
            gpio_params->CONFIG = 0x02;
        }
    }

    /* configure the pin */
    uint32_t tmp = ( (gpio_params->CONFIG << 2) | gpio_params->MODE );
